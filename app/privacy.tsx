...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, StatusBar, Share, Alert, Clipboard } from 'react-native';
import { useRouter } from 'expo-router';

export default function PrivacyScreen() {
  const router = useRouter();

  // 🚀 FUNGSI BARU: Salin Seluruh Isi Kebijakan Privasi (Poin 1 s/d 4) Tanpa Ada yang Terlewat
  const handleCopyPrivacyPolicy = () => {
    const fullPrivacyText = `KEBIJAKAN PRIVASI GARASIKU
Terakhir Diperbarui: Mei 2026

1. Pengumpulan Data & Penyimpanan
GarasiKu adalah aplikasi berbasis Local-First Database. Seluruh data profil pengguna, spesifikasi kendaraan, nomor plat, odometer, log pengisian BBM, hingga foto nota perbaikan disimpan seutuhnya di dalam memori internal perangkat fisik Anda menggunakan enkripsi penyimpanan terisolasi.
...
3. Keamanan Data & Cadangan (Backup)
...
Anda memiliki hak kontrol mutlak 100% atas data Anda sendiri. Anda dapat mengubah, memodifikasi, mengekspor, serta menghapus seluruh data catatan log garasi kapan saja secara instan melalui tombol "Hapus Semua Data Aplikasi" pada menu pengaturan profil Anda.`;
    ...
    try {
      Clipboard.setString(fullPrivacyText);
      Alert.alert(
        "Berhasil Disalin", 
        "Seluruh isi teks Kebijakan Privasi (Poin 1 s/d 4) telah berhasil disalin ke papan klip perangkat Anda."
      );
    } catch (error) {
      Alert.alert("Gagal Menyalin", "Terjadi kesalahan saat mengakses papan klip sistem.");
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {/* 🚀 AKSI TOMBOL HEADER: Berjejer Rapi (Salin & Share) */}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { color: '#FFF', fontSize: 18, fontWeight: '900', flex: 1, textAlign: 'center', marginLeft: 30 },
